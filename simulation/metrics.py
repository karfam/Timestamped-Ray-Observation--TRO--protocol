...
def summarize_time_series(df: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        ...
        "total_gated_rejected_observations": int(df["gated_rejected_observations"].sum()),
        "mean_gated_rejected_observations": float(df["gated_rejected_observations"].mean()),
        "max_gated_rejected_observations": int(df["gated_rejected_observations"].max()),
